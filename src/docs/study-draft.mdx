    ...
    const draggable: Draggable<State> = ({ state, d }) => (
      <g transform={translate(60, 30)}>
        ...
        <circle
          ...
          data-on-drag={() =>
            state.name === "r"
            ? d.between({ name: "r" }, { name: "b" })
            : state.name === "b"
            ? d.between({ name: "b" }, { name: "g" })
            : d.between({ name: "g" }, { name: "r" })
          }
        />
      </g>
    );
    ...
    const draggable: Draggable<State> = ({ state, d }) => (
      <g transform={translate(30, 60)}>
        ...
        <circle
          ...
          fill="white" stroke="#d1d5db" stroke-width={1.5}
          style={{ filter: "drop-shadow(0 1px 3px rgba(0,0,0,0.15))" }}
          data-on-drag={() => d.vary(state, ["value"], constraint: [(s) => s.value])}
        />
      </g>
    );
`}
...
TODO: Be more incremental first?
