...
This defines a "draggable" – a particular kind of function that returns React SVG elements using JSX. It looks an awful lot like a React component function, but it isn't one; sorry; no using hooks or anything like that.

But this draggable doesn't actually do anything. Let's fix that. For a diagram to be interactive, it must have multiple states it can be in. So we will define a state type and an initial state to start at.
...
<LiveEditor
  code={`
    ...
    export const initialState = { status: "off" };

    export const draggable = ({ state, d }) => (
      <circle
...
- Here, we use a `transform` that depends on state – `"off"` will be to the left and `"on"` will be to the right. State is provided as an argument to the draggable.

Now we're ready for the biggest bit: dragging! Dragging is what lets us move into these exciting new states we have. When you attach `dragology` to an SVG element, you are telling DD you want to be able to move between states by dragging the element. You provide a "drag spec" which lists various states that should be accessible by dragging. When you start dragging the element, DD will invisibly render the diagrams of those accessible states, and see where the dragged element ends up. It will use that to synthesize a drag interaction, interpolating between accessible states as you go.

Pieces of this puzzle:

- `dragology` takes a callback that returns a drag spec. The drag spec constructors come from `d`, which is provided as an argument to the draggable.

- Here we use `d.between`, which means "I want to be able to drag back and forth between all the states in this list". There are many alternatives, most prominently `d.floating`, which means the dragged element entirely detaches from the diagram.
...
- Now the important part: the drag spec attached to `dragology`! Dragging an element of the array should be able to bring you to every possible version of the array where the dragged element is removed and then inserted in a different location. We compute all these possible arrays in the drag spec callback. Specifically, for each possible insertion index, we make a copy of the starting state (with `structuredClone`) before mutating and returning it. Alternatively, you could use [Immer](https://immerjs.github.io/immer/)'s `produce` function, or painstakingly use a bunch of `{ ...oldState, newKey: ...}`-type expressions. (Or you could even use `produceAmb` -- search the codebase for examples of that.)
...
The new part here is `d.vary` in the drag spec. This means "the drag should vary a numeric value located at this path in the state object". Specifically, it will control the numeric value to try to make the dragged object as close to the dragged pointer as possible. (That's numeric minimization!) You can pass multiple paths to `d.vary` to control multiple numeric values at once, e.g. `d.vary(state, [["x"], ["y"]])`.
...
If you'd like to more directly move between states, say, in response to a click, you can use `setState`, which is provided as an argument to the draggable. It's more boring than dragging, but it does still have some tricks up its sleeve...
...
<LiveEditor
  code={`
    ...
    export const initialState: State ={ colorIdx: 0 };

    export const draggable: Draggable<State> = ({ state, setState }) => {
